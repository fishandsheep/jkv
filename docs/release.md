...
3. 确认仓库变量 `JKV_CN_DOWNLOAD_BASE` 指向国内 HTTPS 公开目录。
4. 若发布工作流负责上传，配置 `JKV_CN_S3_URI`、endpoint/region 和最小权限凭证。
...
国内目录必须按 tag 隔离且不可覆盖。安装脚本在构建时写入该 tag 的国内下载基址。对象存储公开读取、关闭目录写入、启用版本控制或保留策略。
